...
def clique_order_smallest_order(G: nx.Graph) -> list:
    ...
    for v in clique:
        H.remove_node(v)
    order.extend( strategy_smallest_last(H) )
    return order
...
def coloring_classes(G: nx.Graph, order : list):
    C = greedy_coloring(G, order)
...
def reorder_classes(G: nx.Graph, classes : list[list]):
    ...
    for cls in reversed(classes):
        ...
        for v in cls:
            if all( v in G[u] for u in clique):
                finding = True
                clique.append(v)
                clique_in_classe.append(cls)
                break
        if not finding:
            clique_out_classe.append(cls)
    
    clique_in_classe.extend(clique_out_classe)


    return clique, clique_in_classe

def get_clique(G: nx.Graph, classes : list[list]):
...
def classes_to_order(G: nx.Graph, classes : list[list]):
    n = len(classes)
    order = []
    for cls in classes:
        for v in cls:
...
def max_clique(G: nx.Graph, k : int = 10):
    ...
    for i in range(k):
        #print("iteracao ", i)
        classes = coloring_classes(G, order)
        #print("coloracao_size ", len(classes))
        clique = get_clique(G, classes)
        #print("clique size ", len(clique))
        classes.reverse()
        order = classes_to_order(G, classes)
...
def max_clique2(G: nx.Graph, k : int = 10):
    order = strategy_smallest_last(G)
    ...
    for i in range(k):
        ...
        #print("coloracao_size ", len(classes))
        clique, classes = reorder_classes(G, classes)
        #print("clique : ", clique)
        #print("classes: ", classes)
        order = classes_to_order(G, classes)
        #print("ordering", order)
        
        #print("coloracao size ", len(classes))
        #print("clique size" , len(clique))
...
def large_clique_size(G):
    ...
    *d* is the maximum degree.
    ...
    def _clique_heuristic(G, C, U, size, best_size):
        if not U:
            return max(best_size, size), C
        u = max(U, key=degrees)
        U.remove(u)
        C.append(u)
        N_prime = {v for v in G[u] if degrees[v] >= best_size}
        return _clique_heuristic(G, C, U & N_prime, size + 1, best_size)
    ...
    for u in nodes:
        ...
        best_size, best_clique = _clique_heuristic(G, neighbors, 1, best_size)
...
def branch_and_bound4(
    G: nx.Graph,
    ub_func: Callable[[nx.Graph], int]
) -> dict:
    """Compute Γ(G) via branch-and-bound with in-place graph mutation.
    ...
    def expand(H: nx.Graph, C: list[list]) -> None:
        nonlocal LB, bb_nodes, bestC, ub_func
        if len(H) == 0:
            if len(C) > LB:
                LB    = len(C)
                bestC = C
        ...
        if len(C) +  UB_local <= LB:
            return
        
        lb1 = lower_bound(H)
        lb2 = lower_bound2(H)
        LB_local    = max(lb1["lower_bound"], lb2["lower_bound"])
        C_local = lb1["coloring"] if lb1["lower_bound"] >= lb2["lower_bound"] else lb2["coloring"]
    
        if LB_local == UB_local:

            if len(C) + LB_local > LB:
                LB = len(C) + LB_local

                bestC = C
                bestC.extend(C_local)
...
def grundy_bitmask(G: nx.Graph, maxsize: int):
    """
    Computa o número de Grundy Γ(G) de um grafo G usando bitmasks,
    memoização sobre subconjuntos de vértices e podas por limitante superior.

    ABORDAGEM
    ---------
    Implementa DP sobre subsets com enumeração Bron–Kerbosch pivotada,
    memoização via lru_cache e duas podas por upper bound que evitam
    explorar subproblemas cuja contribuição máxima não supera o melhor
    valor global já encontrado (bestGlobal).

    A recorrência central é:
        Γ(G[S]) = 1 + max_{I ∈ MIS(G[S])} Γ(G[S \ I])

    PODAS IMPLEMENTADAS
    -------------------
    Ambas operam sobre ``bestGlobal``: o maior valor de Γ já confirmado
    em qualquer chamada completada de solve(), inicializado em 0.

    Poda 1 — tamanho do subconjunto:
        Se depth + |S| <= bestGlobal, então mesmo colorindo cada vértice
        de S com uma cor distinta (pior caso = Γ(K_n) = n), o total
        depth + Γ(G[S]) não pode superar bestGlobal. Poda.

    Poda 2 — grau máximo do subgrafo induzido:
        O upper bound clássico Γ(H) ≤ Δ(H) + 1 vale para qualquer grafo H.
        Se depth + Δ(G[S]) + 1 <= bestGlobal, o subproblema não melhora
        o resultado. Poda.
        Δ(G[S]) é computado em O(n) via popcount (bin().count) sobre
        adj[v] & S para cada v ∈ S.

    NOTA: o campo "model" retorna "BB + Bit + Pruning". O contador
    ``bb_nodes`` mede nós do Bron–Kerbosch; podas reduzem chamadas a
    solve(), não diretamente esse contador.

    Parameters
    ----------
    G : networkx.Graph
        Grafo simples e não-direcionado com vértices de rótulo hashável.
    maxsize : int
        Tamanho máximo do lru_cache. Passe None para cache ilimitado.

    Returns
    -------
    dict com as chaves:
        "model"        : str   — identificador do método.
        "gamma"        : int   — número de Grundy Γ(G).
        "cpu_s"        : float — tempo de CPU em segundos.
        "classes"      : list[list] — coloração de Grundy (independentes
                         maximais por nível de cor, com rótulos originais).
        "valid"        : bool  — True se a coloração é de Grundy válida.
        "bb_nodes"     : int   — nós visitados pelo Bron–Kerbosch.
        "pruned"       : int   — chamadas a solve() eliminadas pelas podas.
        "hits"         : int   — acertos no lru_cache.
        "misses"       : int   — faltas no lru_cache.
        "hit_rate"     : float — hits / (hits + misses).
        "memoria_pico" : int   — pico de memória em bytes (tracemalloc).

    Notes
    -----
    Complexidade de tempo: O(3^n) no pior caso sem podas; as podas podem
    reduzir significativamente a exploração em grafos esparsos ou com Δ
    pequeno. A memoização garante que cada subconjunto S é computado
    no máximo uma vez.

    Complexidade de espaço: O(2^n) para a tabela de memoização.
    Para n ≳ 25 o consumo de memória pode se tornar proibitivo.

    Examples
    --------
    >>> import networkx as nx
    >>> G = nx.path_graph(4)               # P4: Γ = 3
    >>> grundy_bitmask(G, None)["gamma"]
    3
    >>> G = nx.complete_graph(4)           # K4: Γ = 4
    >>> grundy_bitmask(G, None)["gamma"]
    4
    >>> G = nx.complete_bipartite_graph(3, 3)  # K_{3,3}: Γ = 3
    >>> grundy_bitmask(G, None)["gamma"]
    3
    ...
    ------
    MemoryError
        Para n ≳ 25 a tabela de memoização pode esgotar a memória.
    """

    # ------------------------------------------------------------------ #
    # 1. PREPARAÇÃO: reindexação e representação bitmask da adjacência    #
    # ------------------------------------------------------------------ #
    nodes = list(G.nodes())
    ...
    # ------------------------------------------------------------------ #
    # 2. UPPER BOUND AUXILIAR: Δ(G[S])                                   #
    #                                                                     #
    # Retorna o grau máximo no subgrafo induzido por S.                   #
    # Para cada vértice v ∈ S, seu grau em G[S] é |adj[v] & S|,          #
    # contado via popcount (bin().count('1')).                            #
    # Custo: O(n) por chamada — aceitável pois ocorre apenas quando       #
    # a Poda 1 falha.                                                     #
    # ------------------------------------------------------------------ #
    def delta_max(S: int) -> int:
    ...
    bb_nodes   = 0   # nós visitados pelo Bron–Kerbosch
    pruned     = 0   # chamadas a solve() eliminadas pelas podas
    bestGlobal = 0   # melhor Γ confirmado globalmente (atualizado ao retornar)
    choice     = {}  # choice[S] = bitmask do melhor independente maximal de S

    @functools.lru_cache(maxsize=maxsize)
    def solve(S: int) -> int:
        """
        Retorna Γ(G[S]) via Bron–Kerbosch com memoização.
        As podas são aplicadas pelo chamador antes de invocar solve().
        """
        nonlocal bb_nodes, bestGlobal
        ...
        # -------------------------------------------------------------- #
        # Bron–Kerbosch pivotado para enumerar independentes maximais     #
        # de G[S]. Pivô = vértice de menor índice em P ∪ X.              #
        # -------------------------------------------------------------- #
        def expand(R: int, P: int, X: int):
            nonlocal best, bestR, bb_nodes, pruned, bestGlobal
            bb_nodes += 1

            if P == 0 and X == 0:
                # Independente maximal R encontrado: avalia a recorrência
                S_next = S & ~R
                size_next = bin(S_next).count('1')

                # --- Poda 1 aplicada ao subproblema filho ------------- #
                # A profundidade do filho é (depth_atual + 1). Como      #
                # solve() não recebe depth, aproximamos: o filho         #
                # contribuirá no máximo size_next a mais.                #
                # A condição conservadora é: se mesmo Γ(K_{S_next})      #
                # = size_next somado ao nível atual (best+1 cores já     #
                # confirmadas) não bate bestGlobal, pula.                #
                # Usamos best+1 como proxy de depth do filho.            #
                current_depth = best + 1  # cores já confirmadas + esta camada
                if current_depth + size_next <= bestGlobal:
                    pruned += 1
                    return

                # --- Poda 2 aplicada ao subproblema filho ------------- #
                if size_next > 0:
                    delta = delta_max(S_next)
                    if current_depth + delta + 1 <= bestGlobal:
                        pruned += 1
                        return
                ...
                if val > best:
                    best  = val
                    bestR = R
                # Atualiza o melhor global assim que confirmamos um valor
                if best > bestGlobal:
                    bestGlobal = best
                return

            # Escolha do pivô: bit menos significativo de P ∪ X
            u_mask = P | X
            u      = (u_mask & -u_mask).bit_length() - 1

            # Candidatos: vértices de P não-vizinhos do pivô u
    ...
    # ------------------------------------------------------------------ #
    # 4. EXECUÇÃO PRINCIPAL com coleta de métricas                        #
    # ------------------------------------------------------------------ #
    tracemalloc.start()
    start = time.time()
    best  = solve(FULL)
    ...
    # ------------------------------------------------------------------ #
    # 5. RECONSTRUÇÃO DA COLORAÇÃO DE GRUNDY via choice[]                 #
    # ------------------------------------------------------------------ #
    coloring = []
    S = FULL
    while S:
...
def dsatur_grundy2(
    ...
    UB = min( delta_1(G), delta_2(G), stair_factor(G), psi_bound(G))
    total = 0
    bb_nodes = 0
    C = [ [] for i in range(UB)]
    ...
    table = psi_table(G)
    max_degree_plus_one = delta_1(G)
    upper_bound = { nodes[i] : min(UB, table[nodes[i]][max_degree_plus_one])  for i in range(n)}
    #print(upper_bound)
    ...
    def expand(
        ...
        nonlocal LB, bestC, total, bb_nodes, pos
        if idx == n:
            total += 1
            if is_greedy_coloring(G, C):
...
def dsatur_grundy3(
    ...
    def expand(
        ...
        if idx == n:
            ...
            # pré-computa avail[v] se não mantido incrementalmente
            avail_v = sum(1 for u in nbr[v] if color[u] == -1)


            for i in range( upper_bound[v] ):
                # classes que v precisa que sejam cobertas:
                # são as classes 0..i-1 que v pulou SEM conflito
                # (se pulou por conflito, já estão cobertas por definição)
                classes_sem_conflito_antes_de_i = sum(
                    1 for j in range(i)
                    if not (nbr[v] & C_sets[j])   # v não tem vizinho em j → precisa que um vizinho futuro cubra
                )

                # para a coloração ser Grundy, v precisa de um vizinho em cada
                # uma dessas classes — e só vizinhos não coloridos podem fazer isso
                if classes_sem_conflito_antes_de_i > avail_v:
                    #print("classes que precisam ser coberta %d" % (classes_sem_conflito_antes_de_i) )
                    #print("disponivel para cobrir %d" % (avail_v) )
                    return    # ← essa atribuição é inviável, pula
...
def solver_carvalho_representante3(
    ...
    *order*) and which variables x[u, v] are created
    ...
    phi = {v: solver.NumVar(0, float(UB), f"phi_{v}" ) for v in nodes}
    ...
    for u in nodes:
        for v in nodes:
            if u != v:
                ...
                solver.Add(phi[u] - phi[v] + 1 <= UB * (1 - y[u, v]))
    ...
    # nós explorados na árvore de B&B
    nodes_explored = solver.nodes()

    # valor da relaxação LP no nó raiz (já calculado por get_linear_relaxation)
    # primeira solução inteira encontrada = objective value se FEASIBLE no primeiro nó
    # proxy: diferença entre relaxação e solução inteira mede qualidade do incumbente inicial
    lp_gap = None
    if feasible and gamma is not None:
        lp_val = get_linear_relaxation(solver)
        lp_gap = (lp_val - gamma) / lp_val if lp_val > 0 else 0.0
    ...
    return {
        ...
        "nodes_explored":    solver.nodes(),   # nós da árvore B&B
        "n_variables":       solver.NumVariables(),
        "n_constraints":     solver.NumConstraints(),
        "lp_gap":            lp_gap,           # (relax - gamma) / relax
    }
...
def solver_carvalho_representante4(
    ...
    # ------------------------------------------------------------------ #
    # Relaxação LP (GLOP) — antes do MIP, modelo idêntico com NumVar      #
    # ------------------------------------------------------------------ #
    ...
    * **Chvátal graph**: Γ=5. [Sampling-verified; 12-vertex 4-regular]
    ...
    for name, G, chi_exp in tests:
        ...
        for _, solver, ub_func, order_func in solvers:
            if solver == grundy_bitmask:
                r = solver(G)
            elif solver == branch_and_bound3:
                r = solver(G, ub_func)
            elif solver == branch_and_bound4:
                r = solver(G, ub_func)
            elif solver == solver_carvalho_representante4:
...
def run_performance_tests(solvers: list) -> str:
    ...
    model_names = [name for name, _, _ in solvers]
    ...
    for name, G in perf:
        ...
        for _, solver, param in solvers:
            ...
            # Limpa o cache entre instâncias para que hits/misses
            # reflitam apenas o grafo corrente, não execuções anteriores.
            if hasattr(solver, "cache_clear"):
                solver.cache_clear()
            ...
            r = dict(r)  # cópia para evitar mutação do resultado original

            # Sobrescreve cpu_s com a medição externa, que exclui o
            # overhead interno de tracemalloc do solver.
            r["cpu_s"]   = end - start
            r["hits"]    = r.get("hits", 0)
            r["misses"]  = r.get("misses", 0)
            # Pico vem do solver, que o mede de forma isolada.
            # Não remedir aqui evita contaminar com alocações do benchmark.
            r["memoria_pico"] = r.get("memoria_pico", 0)

            row_results.append(r)

        gamma = row_results[0]["gamma"]
        # Valida consistência entre solvers: todos devem concordar em Γ.
        ...
        for r in row_results:
            row += [
                ...
                str(r["hits"]),
                str(r["misses"]),
                f"{r['memoria_pico'] / 1024:.1f}",  # bytes → KB
            ]
...
def upper_bound1(G):
    return min(delta_1(G), delta_2(G), stair_factor(G), psi_bound(G))

def upper_bound2(G):
    return min(delta_1(G), delta_2(G), stair_factor(G), revised_stair_factor2(G), psi_bound(G))

def teste_limite_superiores(
        n_samples : int, 
        ub_func1: Callable[[nx.Graph], int],
        ub_func2: Callable[[nx.Graph], int]
    ):
    """
    Testes realizados com 5000 grafos gerados de maneira aleatória
    O limite superior adicionando o upper_bound2  superou upper_bound1 em 20 instâncias 
    representando 0.4% das instancias
    """
    wins1 = 0
    wins2 = 0
    draw = 0
    ...
    for i in range(n_samples):
        ...
        G = nx.erdos_renyi_graph(50, random.choice(p), i)
        UB1 = ub_func1(G)
        UB2 = ub_func2(G)
    ...
    print("limite superior anterior ", wins1)
    print("limite superior atualizado ", wins2)
    print("empate ", draw)


def teste_total_cores(n_samples : int = 5000):
    total_cores1 = 0
    total_cores2 = 0
    wins = 0
    draw = 0
    ...
    for i in range(n_samples):
        ...
        cores1 = 0
        cores2 = 0
        UB = min(delta_1(G), delta_2(G), stair_factor(G), revised_stair_factor2(G), psi_bound(G))
        Psi = psi_table(G)
        delta = delta_1(G)
        for v in G:
            Kv = min( UB , Psi[v][delta])
            cores1 += Kv

            V = set(G.nodes())
            V.remove(v)
            H = G.subgraph(V)
            Kv = min( UB, stair_factor(H) + 1)
            cores2 += Kv
    ...
    print("vitorias ", wins)
    print("emaptes ", draw)
    print("cores1 ", total_cores1)
    print("cores2 ", total_cores2)
    
def teste_total_cores2(n_samples : int = 5000):
    total_cores1 = 0
    total_cores2 = 0
    wins = 0
    draw = 0
...
if __name__ == "__main__":
    ...
    solvers = [
        ("$s=None$",          grundy_bitmask,  None),
        ("$s=10^5$",          grundy_bitmask,  100000),
        ("$s=10^6$",          grundy_bitmask,  1000000),
        
        
        #("BB sf",          branch_and_bound3,                stair_factor, None),
        #("BB sf fast",          branch_and_bound3,                fast_stair_factor, None),
        #("BB psi",          branch_and_bound3,                psi_bound, None),
        #("BB2",          branch_and_bound4,                stair_factor, None),
        
        #("BB",          branch_and_bound3,                revised_stair_factor_fast2, None),
        #("DSATUR BASE" , dsatur_grundy, stair_factor, None),
        #("DSATUR2 SL" , dsatur_grundy2, stair_factor, strategy_smallest_last),
        #("DSATUR LF" , dsatur_grundy2, stair_factor, strategy_largest_first), 
        #("DSATUR3 SL" , dsatur_grundy3, stair_factor, strategy_smallest_last), 
        ...
        #("CarvRepSL", solver_carvalho_representante4,   upper_bound1,
    ...
    #G = nx.erdos_renyi_graph(12, 0.9)

    #r = branch_and_bound3(G)
    #print("time ", r["cpu_s"])
    #print("bb_nodes", r["bb_nodes"])

    #r = branch_and_bound4(G)
    #print("time ", r["cpu_s"])
    #print("bb_nodes", r["bb_nodes"])


    # from experiment_analysis import run_experiment_v3

    # summary = run_experiment_v3(
    # heuristic_function   = clique_order_smallest_order,
    # num_graphs           = 20, n=15, p=0.5, k_perms=30,
    # save_dir             = "results",
    # experiment_tag       = "smallest_last",
    # solver_fn            = solver_carvalho_representante4,
    # stair_factor_fn      = stair_factor,
    # sample_permutations_fn = sample_permutations,
    # )

    
    # summary = run_experiment_v3(
    # heuristic_function   = strategy_smallest_last,
    # num_graphs           = 20, n=15, p=0.5, k_perms=30,
    # save_dir             = "results",
    # experiment_tag       = "smallest_last",
    # solver_fn            = solver_carvalho_representante4,
    ...
    #teste_limite_superiores()
    #teste_total_cores(200)
    #teste_total_cores2(200)
    ...
    with open("./table/tabela.tex", "w") as f:
        f.write(latex_code)
